...
<template>
    <div class="max-w-5xl mx-auto mt-6">
        ...
        <!-- Post Form -->
        <PostForm v-if="showForm" :modelValue="selectedPost" :isEdit="!!selectedPost" @submitted="handleSubmitted" />
        ...
        <table v-if="postStore.posts.length" class="min-w-full border border-gray-300 mt-6">
            ...
            <tbody>
                <tr v-for="(post, index) in postStore.posts" :key="post.id">
                    ...
                    <td class="border p-2">
                        <img v-if="post.image" :src="`http://127.0.0.1:8000/storage/${post.image}`" alt="Post Image"
                            width="50" />
                    </td>
